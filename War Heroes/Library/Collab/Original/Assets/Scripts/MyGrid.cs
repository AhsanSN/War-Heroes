...
public class MyGrid : MonoBehaviour
{
    ...
    private void checkForAllDeadUnits()
    {
        ...
        {
            print("all my unit dead");
        }

        deadUnits = 0;
        for (int i = 0; i < enemyUnits.Count; i++)
        {
            if (enemyUnits[i].GetComponent<Unit>().DeathStatus)
            {
                deadUnits += 1;
            }
        }
        if (deadUnits == enemyUnits.Count)
        {
            print("all enemy unit dead");
        }
    }
    ...
    private void moveEnemyUnitAI()
    {
        ...
        if (distanceToEnemy(units[randI].transform.position.x, enemyUnits[playerEnemyTurn].GetComponent<Unit>().transform.position.x, units[randI].transform.position.y, enemyUnits[playerEnemyTurn].GetComponent<Unit>().transform.position.y) < enemyUnits[playerEnemyTurn].GetComponent<Unit>().Speed && units[randI].GetComponent<Unit>().DeathStatus==false)
        {
            toPosEnemy[playerEnemyTurn] = units[randI].transform.position;
        }
        else
